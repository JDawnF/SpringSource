 ...
 * 容器可以管理bean的生命周期；我们可以自定义初始化和销毁方法；
 ...
 * 有以下方式管理生命周期：
 * 1）、指定初始化和销毁方法；
 * 		通过@Bean指定init-method和destroy-method,即初始化和销毁方法；
 * 2）、通过让Bean实现下面两个接口：
 ...
 *
 * Spring底层对 BeanPostProcessor 的使用(参照Dog.java):
 * 	bean赋值，注入其他组件，@Autowired，生命周期注解功能，@Async,xxx BeanPostProcessor;
 */

@ComponentScan("com.baichen.bean")
@Configuration
public class MainConfigOfLifeCycle {
    //@Scope("prototype")       多实例
//    指定初始方法和销毁方法，即Car.java中定义的方法init和destroy
