 ...
 * @Author: baichen
 * @Description: AOP配置类,demo:{@link com.baichen.aop.MathCalculator}
 * AOP：【动态代理】指在程序运行期间动态的将某段代码切入到指定方法指定位置进行运行的编程方式；
 * 1、导入aop模块依赖； Spring AOP：(spring-aspects)
 * 2、定义一个业务逻辑类（MathCalculator）:在业务逻辑运行的时候将日志进行打印（方法之前、方法运行结束、方法出现异常，xxx）
 * 3、定义一个日志切面类（{@link com.baichen.aop.LogAspects}）:切面类里面的方法需要动态感知MathCalculator类运行到哪里然后执行；
 * 		通知方法：
 * 			前置通知(@Before)：logStart：在目标方法(div)运行之前运行
 * 			后置通知(@After)：logEnd：在目标方法(div)运行结束之后运行（无论方法正常结束还是异常结束）
 * 			返回通知(@AfterReturning)：logReturn：在目标方法(div)正常返回之后运行
 * 			异常通知(@AfterThrowing)：logException：在目标方法(div)出现异常以后运行
 * 			环绕通知(@Around)：动态代理，手动推进目标方法运行（joinPoint.proceed()），执行之前就是前置通知，执行之后就是后置通知
 * 4、给切面类的目标方法标注何时何地运行（通知注解，即要拦截通知的方法）；
 * 5、将切面类和业务逻辑类（目标方法所在类）都加入到容器中，可以通过在配置类中加入@Bean注解，然后创建这两个类;
 * 6、必须告诉Spring哪个类是切面类(给切面类上加一个注解：@Aspect)
 * 7[最重要的一步]、在配置类中加 @EnableAspectJAutoProxy 【开启基于注解的aop模式】，即启用AspectJ自动代理
 * 		在Spring中很多的 @EnableXXX,表示开启某种功能
 *
 * 写AOP的主要三步：
 * 	1）、将业务逻辑组件和切面类都加入到容器中,告诉Spring哪个是切面类（@Aspect标注在切面类上）
 * 	2）、在切面类上的每一个通知方法上标注通知的相关注解，告诉Spring何时何地运行（切入点表达式，如execution表达式）
 *  3）、开启基于注解的aop模式，@EnableAspectJAutoProxy，如这里是在配置类中标注
 *
 * AOP原理：【@Enablexxx:先看给容器中注册了什么组件，这个组件什么时候工作，这个组件的功能是什么？】
 ...
 * AbstractAutoProxyCreator.有后置处理器的逻辑,即postProcessXxx,都打上断点
 *
 * AbstractAdvisorAutoProxyCreator.setBeanFactory(),在这个方法中调用了initBeanFactory(),初始化bean工厂
 * AnnotationAwareAspectJAutoProxyCreator.initBeanFactory(),在父类中会调用setBeanFactory，所以这里会调用这个方法
 * 整个调用流程：
 ...
 * 			后面可以拦截这些创建过程，做相应的处理
 * 				1）、createBeanInstance方法：创建Bean的实例
 * 				2）、populateBean方法：给bean的各种属性赋值
 * 				3）、initializeBean方法：初始化bean(后置处理器其实就是在初始化bean的前后进行工作)
 * 					1）、invokeAwareMethods()：先判断bean对象是否为Aware接口，如果是则调用对应的Aware接口的方法，即处理Aware接口的方法回调
 ...
 * 					3）、invokeInitMethods()：执行自定义的初始化方法(如init指定的初始化方法)
 * 					4）、applyBeanPostProcessorsAfterInitialization()；执行后置处理器的postProcessAfterInitialization（），与上面的 3 类似
 * 				4）、BeanPostProcessor(以AnnotationAwareAspectJAutoProxyCreator为例)创建成功 --》aspectJAdvisorsBuilder
 * 			7）、把BeanPostProcessor注册到BeanFactory中：beanFactory.addBeanPostProcessor(postProcessor);
 *    ============以上是创建和注册AnnotationAwareAspectJAutoProxyCreator(最终的父类的后置处理器)的过程===================
 * 			两种不同类型的后置处理器：AnnotationAwareAspectJAutoProxyCreator => InstantiationAwareBeanPostProcessor
 *
 ...
 * 				1）、(单例获取到的变量名是sharedInstance，会先声明，即先从缓存中获取当前bean，如果能获取到，说明bean是之前被创建过的，直接使用，否则再创建；只要创建好的Bean都会被缓存起来
 * 				2）、调用createBean() 创建bean ：
 ...
 * 					【InstantiationAwareBeanPostProcessor  是在创建Bean实例之前，先尝试用后置处理器返回对象的】
 * 				   步骤：
 * 					1）、resolveBeforeInstantiation(beanName, mbdToUse);  解析BeforeInstantiation(希望后置处理器在此能返回一个代理对象，如果能返回代理对象就使用，如果不能就继续创建bean)
 * 						1）、后置处理器先尝试返回对象：
 * 							bean = applyBeanPostProcessorsBeforeInstantiation（）：拿到所有后置处理器，如果是InstantiationAwareBeanPostProcessor，就执行postProcessBeforeInstantiation方法
 * 							if (bean != null) {
 ...
 * 		3）、是否需要跳过
 * 		    1）、获取候选的增强器（增强器即切面里面的通知方法）【将通知方法封装为：List<Advisor> candidateAdvisors】
 ...
 *  postProcessAfterInitialization； return wrapIfNecessary(bean, beanName, cacheKey);//需要的话就进行包装，情况如下：
 * 		1）、获取当前bean的所有增强器（通知方法） 即 Object[]  specificInterceptors
 * 			1、找到候选的所有的增强器（找哪些通知方法是需要切入当前bean方法的）
 * 			2、获取到能在当前bean使用的增强器
 * 			3、给增强器排序
 * 		2）、保存当前bean在advisedBeans(缓存)中；
 * 		3）、如果当前bean需要增强，创建当前bean的代理对象,createProxy方法：
 * 			1）、获取所有增强器（通知方法）
 * 			2）、保存到proxyFactory
 * 			3）、通过proxyFactory的getProxy()创建代理对象(Spring自动决定判断)，有以下两种方式：
